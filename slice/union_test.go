...
func TestUnion(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.ElementsMatch(t, test.expected, Union(test.src1, test.src2))
		})
	}
}
...
func TestUnionFunc(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.ElementsMatch(t, test.expected, UnionFunc(test.src1, test.src2, func(left, right int) bool {
				return left == right
			}))
		})
	}
